...
Graph* GraphCopy(const Graph* g) {
  int vertices = g->verticesList;
  int i;
  int j;
  Graph* aux = GraphCreate(g->numVertices,g->isDigraph);
  for (i = 0; i < g->numVertices; ListMoveToNext(vertices),i++){
    struct _Vertex* ver = ListGetCurrentItem(vertices);
    List* aresta = ver->edgesList;
    for (j=0; j < g->numEdges; ListMoveToNext(aresta),j++){
      struct _Edge* are = (struct _Edge*)ListGetCurrentItem(aresta);
      GraphAddEdge(aux,are->adjVertex,are->weight);
    }
...
Graph* GraphFromFile(FILE* f) {
  ...
  if (f == NULL){
    printf("Problemas com leitura de ficheiro\n");
    return 0;
  }
  i = 1;
  while (!feof(f)){
      result = fgets(Linha, 100, f);  // o 'fgets' lê até 99 caracteres ou até o '\n'
