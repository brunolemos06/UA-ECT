...
void ListMove(List* l, int newPos) {
  ...
  if (newPos == -1) {  // move outside
    ...
    if(l->currentPos <= newPos && l->currentPos >= 0){
      for(int i= l->currentPos ; i < newPos ;i++){
        l->current = l->current->next;
      }
    }else{ //  if( l->currentPos > newPos){
      l->current = l->head;
...
int ListSearch(List* l, const void* p) {
  struct _ListNode * aux = l->current;
  int pos = l->currentPos;
  l->current = l->head;

  for(int i = 0 ; i < l->size ; i++){
    if( l->compare(p,l->current->item) == 0 ) {
      l->currentPos = i;
      return 0; //return 0 and move the current node to the found element.
    }
    l->current = l->current->next;
  }
  ...
  l->current = aux;
  l->currentPos = pos;
  return -1; // If search fails. return -1 and don't change the current node.
}
...
void* ListRemoveCurrent(List* l) {
  ...
  if (l->currentPos == 0)
    ...
    struct _ListNode * aux = l->head;
    for(int i = 0; i < l->currentPos -1  ; i++){ // procurar current -1
      aux = aux->next;
    }
    item = aux->next->item;
