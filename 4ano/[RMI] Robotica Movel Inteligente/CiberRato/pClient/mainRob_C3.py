...
class MyRob(CRobLinkAngs):
    ...
    def wander(self):
        global direcao_atual,direita,esquerda,rotate_right,rotate_left,direcao_retaguarda,save,tras,antigo_tras,NodeID,Graph,path,go_to_path_step,direcao,go_to_path,LastvisitedNode,go_front,flagback,ground,arrayGPS_x,arrayGPS_y,direcaosave,checkpointsList
        x = int(round(self.measures.x-self.x))
        y = int(round(self.measures.y-self.y))
        ...
        # print("Checkpoiins->" , checkpointsList)
        if ground > 0:
            direcaosave = direcao_atual
            arrayGPS_x.append(self.measures.x-self.x)
            arrayGPS_y.append(self.measures.y-self.y)
        else:
            try:
                if direcaosave == direcao_atual:
                    ...
                    arrayGPS_x = []
                    arrayGPS_y = []
                    if LastvisitedNode not in checkpointsList:
                        checkpointsList.append(LastvisitedNode)
            except:
                pass
        ...
        if rotate_right:
            ...
            if (values == [0,0,1,1,1,0,0] or values == [0,0,0,1,1,0,0] or values == [0,0,1,1,0,0,0]) and direcao_atual == self.getDireita_Esquerda_Trás(direcao_retaguarda)[0]:
                rotate_right = 0
                if ground > 0 and LastvisitedNode not in checkpointsList:
                    checkpointsList.append(LastvisitedNode)
    ...
    def driveInFront(self,values,final,x,y):
        ...
        if values[0] == 1 and  save[0] == 1 and final == (x,y): # FULL ESQUERDA
            ...
            self.driveMotors(speedy,speedx)
        elif values[3] == 1 and values[4] == 1 and values[5]==1 and values[6]==0: # 00 111 10
    ...
    def getdistanceInGraph(self,node1):
        temp = {}
    ...
    def find_minimum_path(self,x,y,endnode):
        global Graph
    ...
    def drive(self,values,x,y):
        ...
        if deadend == 1: #Resolve Beco-sem-saida
            ...
            if values[0] == 1 and values[1] == 1 and values[2] == 1 and save[0] == 1 and save[1] == 1 and save[2] == 1 and values[6] == 1 and values[5] == 1 and values[4] == 1:
                ...
                self.driveMotors(speedx,speedy)
            elif values == [0,0,0,0,0,0,0]: # DEAD END
                self.driveMotors(-0.6,-0.6)
                deadend = 1
                direcao_retaguarda = direcao_atual
            else:
                self.driveMotors(0.09,0.09) 
        
    def gettras(self,direcao):
        if direcao == norte:
            return sul
        elif direcao == sul:
            return norte
    ...
    def getDireita_Esquerda_Trás(self,v):
        if v == norte:
            direita = este
            esquerda = oeste
            tras = sul
    ...
    def AtualizarDirecaoAtual(self):
    ...
    #Direction dicider function recieves database info of node (x,y)
    ...
    def run(self):
        ...
        while True:
            ...
            if len(checkpointsList) == int(self.nBeacons):
                self.getABC()
    ...
    def MyRobMap(self):
        valuex = int(round(self.measures.x-self.x))
        valuey = int(round(self.measures.y-self.y))
        self.labMap[10][24] = "I"
    ...
    def SaveFileMap(self):
        # save in file "myrob.map" the array self.labMap
