...
public class ControlCollectionSite{
   ...
   */
   public ControlCollectionSite (GeneralRepoStub repo, AssaultPartyStub[] assaultParties) {
   ...
   public int getAssaultPartyID() {
      for (int i = 0; i < assaultPartyInOperation.length; i++) {
         ...
         if (assaultPartyInOperation[i] == 0) {
            ...
            System.out.println("Assault party " + i + " is in operation");
            return i;
         }
      }
      return -1;
   ...
   public int getRoomWithCanvasID() {
      for (int i = 0; i < rooms.length; i++) {
         if(rooms[i] && roomIsTarget[i]==0){
            ...
            roomIsTarget[i] = 3;
            System.out.println("Room " + i + " is a target");
            return i;
         }
      }
      return -1;
   ...
   */
   public synchronized void startOfOperation(){
      ((ControlCollectionSiteClientProxy) Thread.currentThread()).setThiefState(MasterThiefStates.DECIDING_WHAT_TO_DO);
   ...
   */
   public synchronized void handACanvas(int assaultPartyID) {
      int thiefID = ((ControlCollectionSiteClientProxy) Thread.currentThread()).getThiefId();
      try{
         //add info and wait for master thief to collect canvas
         waitingCanvas.write(new Data(assaultPartyID, thiefID, assaultParties[assaultPartyID].getTargetRoom(0), assaultParties[assaultPartyID].getHasCanvas(thiefID))); //insert assaultPartyID, thiefID, roomID and canvas in fifo
      } catch (MemException e) {
      ...
      repo.thiefStateW( ((ControlCollectionSiteClientProxy) Thread.currentThread()).getThiefId());
   }
