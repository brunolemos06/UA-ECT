...
public class AssaultParty {
    ...
    */
     ...
     */
    public AssaultParty(int assaultID,GeneralRepoStub repo) {
    ...
    */
    public synchronized void setAssaultID(int assaultID) {
        this.assaultID = assaultID;
        System.out.println("ASSAULT PARTY ID: " + this.assaultID);
    }
    ...
    */
    public synchronized void setTargetRoom(int targetRoom) {
        ...
        System.out.println("ASSAULT PARTY TARGET ROOM: " + this.targetRoom);
    }
    ...
    */
    public synchronized void setRoomDistance(int roomDistance) {
        ...
        System.out.println("ASSAULT PARTY ROOM DISTANCE: " + this.roomDistance);
    }
    ...
    */
    public synchronized void addThief(){
        for (int i = 0; i < SimulPar.K; i++) {
            if(this.thieves[i] == -1){
                this.thieves[i] = ((AssaultPartyClientProxy) Thread.currentThread()).getThiefId();
                thievesAtParty++;
                notifyAll();
                repo.setThiefsInAssault(assaultID,((AssaultPartyClientProxy) Thread.currentThread()).getThiefId());
                break;
            }
        }
    }
    ...
    */
    public synchronized void reverseDirection(){
        ((AssaultPartyClientProxy) Thread.currentThread()).setThiefState(NormalThiefStates.CRAWLING_OUTWARDS);
        repo.putThiefState(NormalThiefStates.CRAWLING_OUTWARDS, ((AssaultPartyClientProxy) Thread.currentThread()).getThiefId());
        roomDirection =false;
        // start crawling out
        thiefCrawl[0] = 1;
    ...
    */
    public synchronized void crawlIn(){
        int walk = -1;
        int id = tranformID(((AssaultPartyClientProxy) Thread.currentThread()).getThiefId());
        System.out.println("PROXY_ID: " + ((AssaultPartyClientProxy) Thread.currentThread()).getThiefId() + " ON CRAWL IN");
        while(true){
            // wait until crawlin == 1
            // System.out.print("PROXY_ID: " + ((AssaultPartyClientProxy) Thread.currentThread()).getThiefId() + " HELOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOO\n");
            ...
            while(canImove(walk)){
                ...
                id = tranformID(((AssaultPartyClientProxy) Thread.currentThread()).getThiefId());
                ...
                if(walk != 0){
                    repo.changeposition(tranformID(((AssaultPartyClientProxy) Thread.currentThread()).getThiefId()), assaultID, printthives);
                }
            }
            ...
            walk = -1;
            if (finish()){
                thievesPosition[tranformID(((AssaultPartyClientProxy) Thread.currentThread()).getThiefId())] =  this.roomDistance;
                thieves[tranformID(((AssaultPartyClientProxy) Thread.currentThread()).getThiefId())] = ((AssaultPartyClientProxy) Thread.currentThread()).getThiefId();
                System.out.println("PROXY_ID : " + ((AssaultPartyClientProxy) Thread.currentThread()).getThiefId() + " CRAWLED IN FINISHED ");
                break;
            }
        }
        ((AssaultPartyClientProxy) Thread.currentThread()).setThiefState(NormalThiefStates.AT_A_ROOM);
        repo.putThiefState(NormalThiefStates.AT_A_ROOM, ((AssaultPartyClientProxy) Thread.currentThread()).getThiefId());
        tryToWakeAll();
    }
    ...
    */
    public synchronized void crawlOut(){
        int walk = -1;
        int id = tranformID(((AssaultPartyClientProxy) Thread.currentThread()).getThiefId());
        System.out.println("PROXY_ID: " + ((AssaultPartyClientProxy) Thread.currentThread()).getThiefId() + " ON CRAWL OUT");
        while(true){
            // wait until crawlin == 1
            while(thiefCrawl[id] == 0){
                try{
            ...
            if (finish()){
                thievesPosition[tranformID(((AssaultPartyClientProxy) Thread.currentThread()).getThiefId())] =  this.roomDistance;
                thieves[tranformID(((AssaultPartyClientProxy) Thread.currentThread()).getThiefId())] = ((AssaultPartyClientProxy) Thread.currentThread()).getThiefId();
                System.out.println("PROXY_ID : " + ((AssaultPartyClientProxy) Thread.currentThread()).getThiefId() + " CRAWLED OUT FINISHED ");
                break;
            }
        }
        tryToWakeAll();
    ...
    */
    public synchronized void sendAssaultParty(){
        ...
        ((AssaultPartyClientProxy) (Thread.currentThread())).setMasterThiefState(MasterThiefStates.DECIDING_WHAT_TO_DO);
    ...
    */
    ...
    private int moveTheMostICan(){
        int walk = ((AssaultPartyClientProxy) Thread.currentThread()).getThiefAgility();
        while(walk!=0){
    ...
    private void tryToWakeAll(){
        ...
        if(!roomDirection){
            ((AssaultPartyClientProxy) Thread.currentThread()).setThiefState(NormalThiefStates.COLLECTION_SITE);
            repo.putThiefState(NormalThiefStates.COLLECTION_SITE, ((AssaultPartyClientProxy) Thread.currentThread()).getThiefId());
        }
        for(int i = 0; i < SimulPar.K;i++){
