...
public class Heist {
    ...
    public static void main(String[] args) {
        ...
        repo = new GeneralRepo(fileName);//fileName);
        repo.header();
        for (int i = 0; i < SimulPar.N_PARTIES; i++) {
            assaultParty[i] = new AssaultParty(i,repo);
        }
        controlCollectionSite = new ControlCollectionSite(repo);
        ...
        repo.setroomdist(roomDistance);
        repo.setPaiting(paintings);
        ...
        for (int i = 0; i < SimulPar.M-1; i++) {
            ...
            thieves[i] = new NormalThief(i, agility,controlCollectionSite, concentrationSite, assaultParty, repo, museum);
            agilities[i] = agility;
        }
        repo.addAgility(agilities);

        masterThief = new MasterThief(controlCollectionSite, concentrationSite, assaultParty, repo);
