...
public class ControlCollectionSite{
   ...
   */
   public ControlCollectionSite (GeneralRepo repo) {
   ...
   */
   public synchronized void takeARest() {
      ((MasterThief) Thread.currentThread()).setThiefState(MasterThiefStates.WAITING_FOR_ARRIVAL);
   ...
   */
   public synchronized void handACanvas(int assaultPartyID) {
      ...
      try{
         ...
         waitingCanvas.write(new Data(assaultPartyID, thiefID, ((NormalThief) Thread.currentThread()).getTargetRoomID() , ((NormalThief) Thread.currentThread()).getHasCanvas())); //insert assaultPartyID, thiefID, roomID and canvas in fifo
      } catch (MemException e) {
      ...
      ((NormalThief) Thread.currentThread()).setHasCanvas(false); //set thief as not having canvas
      repo.changehasacanvas(thiefID, 0,-1,assaultPartyID);
      repo.thiefStateW( ((NormalThief) Thread.currentThread()).getThiefId());
   }
}