...
public class GeneralRepo {
    ...
    public synchronized void finish(int totalcanvas){
        ...
        try {
            ...
            writer.write("Thief # S    – situation of the ordinary thief # (# - 1 .. 6) either 'W' (waiting to join a party) or 'P' (in party)\n");
            writer.write("Thief # MD   – maximum displacement of the ordinary thief # (# - 1 .. 6) a random number between 2 and 6\n");
            writer.write("Assault party # RId        – assault party # (# - 1,2) elem # (# - 1 .. 3) room identification (1 .. 5)\n");
            writer.write("Assault party # Elem # Id  – assault party # (# - 1,2) elem # (# - 1 .. 3) member identification (1 .. 6)\n");
            writer.write("Assault party # Elem # Pos – assault party # (# - 1,2) elem # (# - 1 .. 3) present position (0 .. DT RId)\n");
            writer.write("Assault party # Elem # Cv  – assault party # (# - 1,2) elem # (# - 1 .. 3) carrying a canvas (0,1)\n");
            writer.write("Museum Room # NP - room identification (1 .. 5) number of paintings presently hanging on the walls\n");
            writer.write("Museum Room # DT - room identification (1 .. 5) distance from outside gathering site, a random number between 15 and 30\n");
            writer.close();
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    // get and set methods
    public synchronized void addAgility(int[] thievesAgility){
        // add if position is null
        this.thievesAgility = thievesAgility;
    }
    ...
    public synchronized void setroomdist(int[] roomaxdist){
        this.roomaxdist = roomaxdist;
    }
    ...
    public synchronized void setPaiting(int[] paiting){
        for(int i=0;i<5;i++){
            this.roomPaintings[i] = paiting[i];
        }
    }
