...
// create threads
static pthread_t* thread_patient;
static pthread_t* thread_nurses;
static pthread_t* thread_doctors;
static pthread_mutex_t* mutex_patient;
static pthread_cond_t* cond_patient;
/**
...
void init_simulation(uint32_t np,uint32_t nn,uint32_t nd)
{
   ...
   hd->num_patients = np;
   thread_patient = new pthread_t[np];
   thread_nurses = new pthread_t[nn];
   thread_doctors = new pthread_t[nd];
   mutex_patient = new pthread_mutex_t[np];
   cond_patient = new pthread_cond_t[np];
   for (size_t i = 0; i < np; i++){
      mutex_patient[i] = PTHREAD_MUTEX_INITIALIZER;
      cond_patient[i]  = PTHREAD_COND_INITIALIZER;
   }
...
void* doctor_life(void* argv){
   while(1){
      ...
      mutex_lock(&mutex_patient[patient]);
      hd->all_patients[patient].done = 1;
      cond_broadcast(&cond_patient[patient]);
      mutex_unlock(&mutex_patient[patient]);
   }
   return NULL;
}
...
{
   mutex_lock(&mutex_patient[id]);
   check_valid_name(hd->all_patients[id].name);
   while(hd->all_patients[id].done !=1)
   {
      cond_wait(&cond_patient[id],&mutex_patient[id]);
   }
   printf("\e[30;01mPatient %s (number %u): health problems treated\e[0m\n", hd->all_patients[id].name, id);
   
   mutex_unlock(&mutex_patient[id]);
}
...
int main(int argc, char *argv[])
{
   ...
   for (size_t i = 0; i < npatients; i++)
   {
      ...
      *id = i;
      pthread_create(&thread_patient[i],NULL,patient_life,(void*) id);
   }

   for (size_t i = 0; i < nnurses; i++)
   {
      pthread_create(&thread_nurses[i],NULL,nurses_life,NULL);
   }

   for (size_t i = 0; i < ndoctors; i++)
   {
      pthread_create(&thread_doctors[i],NULL,doctor_life,NULL);
   }
   ...
   for (size_t i = 0; i < nnurses; i++)
   {
      pthread_detach(thread_nurses[i]);
   }

   for (size_t i = 0; i < ndoctors; i++)
   {
      pthread_detach(thread_doctors[i]);
   }
